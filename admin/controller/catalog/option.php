...
class ControllerCatalogOption extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_catalog_option->addsizeOption($this->request->post);
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_catalog_option->editsizeOption($this->request->get['option_id'], $this->request->post);
	...
	protected function getForm() {
		...
		$data['text_size_option'] = $this->language->get('text_size_option');
		$data['entry_image'] = $this->language->get('entry_image');
		$data['entry_discription'] = $this->language->get('entry_description');
		$data['entry_remarks'] = $this->language->get('entry_remarks');
		$data['entry_size_value'] = $this->language->get('entry_size_valuee');
		...
		if (isset($this->request->post['option_description'])) {
			...
			$data['option_description'] = $this->model_catalog_option->getsizeOptionDescriptions($this->request->get['option_id']);
		} else {
			$data['option_description'] = array();
		}
		$this->load->model('tool/image');
		if (isset($this->request->post['option_image'])) {
			$data['option_image'] = $this->request->post['option_image'];
			$data['thumb'] = $this->model_tool_image->resize($this->request->post['option_image'], 100, 100);
		} elseif (!empty($option_info['image'])) {
			$data['option_image'] = $option_info['image'];
			$data['thumb'] = $this->model_tool_image->resize($option_info['image'], 100, 100);
		} else {
			$data['option_image'] = 'sizeoption.jpg';
			$data['thumb'] = $this->model_tool_image->resize('sizeoption.jpg', 100, 100);
		}
		...
		if (isset($this->request->post['option_value'])) {
			...
			$option_values = $this->model_catalog_option->getsizeOptionValueDescriptions($this->request->get['option_id']);
		} else {
			$option_values = array();
		}
		...
		foreach ($option_values as $option_value) {
			...
			if(isset($option_value['bust'])){
				$bust = $option_value['bust'];
			} else {
				$bust = '';
			}
            if(isset($option_value['waist'])){
				$waist = $option_value['waist'];
			} else {
				$waist = '';
			}
            if(isset($option_value['hip'])){
				$hip = $option_value['hip'];
			} else {
				$hip = '';
			}
            if(isset($option_value['hollow'])){
	...
	protected function validateForm() {
		...
		if (($this->request->post['type'] == 'select' || $this->request->post['type'] == 'radio' || $this->request->post['type'] == 'size_option' || $this->request->post['type'] == 'checkbox') && !isset($this->request->post['option_value'])) {
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name'])) {
			...
			foreach ($options as $option) {
				...
				if ($option['type'] == 'select' || $option['type'] == 'radio' || $option['type'] == 'checkbox' || $option['type'] == 'size_option' || $option['type'] == 'image') {
