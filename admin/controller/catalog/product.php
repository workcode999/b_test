...
class ControllerCatalogProduct extends Controller {
	...
	protected function getForm() {
		...
		if ($this->db->query("SHOW TABLES LIKE '". DB_PREFIX ."product_promotional_info'")->num_rows == 0) {
	            $sql = "CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "product_promotional_info` (
					  `product_id` int(11) NOT NULL,
					  `language_id` int(11) NOT NULL,
					  `extra_field` VARCHAR(1000) NOT NULL
					) ENGINE=MyISAM COLLATE=utf8_general_ci";
	            $this->db->query($sql);          
	      }
					if (isset($this->request->post['product_promotional_info'])) {
			$data['product_promotional_info'] = $this->request->post['product_promotional_info'];
		} elseif (isset($this->request->get['product_id'])) {
			$data['product_promotional_info'] = $this->model_catalog_product->getProductExtraField($this->request->get['product_id']);
		} else {
			$data['product_promotional_info'] = array();
		}
