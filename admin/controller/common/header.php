	...
	public function index() {
		...
		if (!isset($this->request->get['token']) || !isset($this->session->data['token']) && ($this->request->get['token'] != $this->session->data['token'])) {
			...
			if ($this->config->get('qa_status')) {
				...
				$data['text_unanswered_questions'] = $this->language->get('text_unanswered_questions');
				$total_unanswered_questions = (int)$this->model_catalog_qa->getTotalQuestionsAwaitingAnswer();
				$data['total_unanswered_questions'] = $total_unanswered_questions;
				$data['qa'] = $this->url->link('catalog/qa', 'token=' . $this->session->data['token'] . '&filter_status=0', true);
			}
			...
			$data['alerts'] = $total_unanswered_questions + $customer_total + $product_total + $review_total + $return_total + $affiliate_total;
