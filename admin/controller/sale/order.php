	...
	public function export() {
        ...
        $orders_list = array();
			...
			$orders_list[] = array(
				...
				'email'			=> $result['email'],
				'telephone'		=> $result['shipping_custom_field'],
...
//var_dump($filter_order_status);die;
		if (isset($this->request->get['filter_total'])) {
		...
		foreach ($results as $result) {
			$data['orders'][] = array(
				'review_booster' => (substr($result['store_url'], -1) == '/' ? $result['store_url'] : $result['store_url'] . '/') . 'index.php?route=product/review_booster/cron',
				'order_id'      => $result['order_id'],
	...
	public function invoice() {
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info) {
				...
				$fieldData = $this->model_sale_xcustom_field->getCustomFieldByIdentifier($order_info['custom_field'],'account');
				foreach($this->model_sale_xcustom_field->getCustomFieldsAll($order_info['customer_group_id']) as $field){
				...
				$formatted_address = $this->model_sale_xcustom_field->getFormattedAddressForOrder($order_info);
		...
		if ($pdf){
		 	$this->response->setOutput(pdf($this->load->view('sale/order_invoice.tpl', $data),$data));
