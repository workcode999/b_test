...
class ControllerSettingSetting extends Controller {
	private $error = array();
	public function ipsfeature() {
				
		$vqmod_file = DIR_SYSTEM . '../vqmod/xml/' . $this->request->post['vqmod'] . '.xml';
		if (is_file($vqmod_file . '_')) {
			rename($vqmod_file . '_', $vqmod_file);

			$json['success'] = 'Activate Success';
		} else if (is_file($vqmod_file)) {
			rename($vqmod_file, $vqmod_file . '_');

			$json['success'] = 'Disable Success';
		} else {
			$json['error'] = 'File Location Error';
		}
	...
	public function runpagespeed() {
		if($_GET["function"] == "curl") {
			...
			if (empty($buffer)) { // check for success or failure
	...
	private function saveMinifyConfig($post) {
		$output = '<' . '?php' . "\n";
		$output .= '////////////////////////////////////////////////////////////////////////////////////////////' . "\r\n";
		$output .= '//    AUTOMATICALLY GENERATED CONFIGURATION FILE FOR MINIFICATION PROCESSING LIBRARIES    //' . "\r\n";
		$output .= '////////////////////////////////////////////////////////////////////////////////////////////' . "\r\n";
		$output .= '$min_encodeImages           = ' . (($post['config_minify_encode_images']) ? 'true' : 'false') . ';' . "\r\n";
		$output .= '$min_encodeImageSize        = ' . (($post['config_minify_image_size']) ? $post['config_minify_image_size'] : '3072') . ';' . "\r\n";
		$output .= '$min_encodeImageOccurs      = ' . (($post['config_minify_image_occurs']) ? $post['config_minify_image_occurs'] : '3') . ';' . "\r\n";
		$output .= '$min_encodeURL              = ' . (($post['config_minify_encode_url']) ? 'true' : 'false') . ';' . "\r\n";
		$output .= '$min_errorLogger            = ' . (($post['config_minify_logging']) ? 'true' : 'false') . ';' . "\r\n";
		$output .= '$min_allowDebugFlag         = ' . (($post['config_minify_debug_mode']) ? 'true' : 'false') . ';' . "\r\n";
		$output .= '$min_serveOptions[\'maxAge\'] = ' . (($post['config_minify_max_age']) ? $post['config_minify_max_age'] : '86400') . ';' . "\r\n";
		$output .= '$min_documentRoot           = \'' . (($post['config_minify_file_path']) ? $post['config_minify_file_path'] : '') . '\';' . "\r\n";
		$output .= '$min_cacheFileLocking       = ' . $post['config_minify_file_locking'] . ';' . "\r\n";
		if (isset($post['config_minify_storage']) && $post['config_minify_storage']) {
			$output .= 'require dirname(__FILE__) . \'/Minify/Cache/APC.php\';' . "\r\n";
			$output .= '$min_cachePath = new Minify_Cache_APC();' . "\r\n";
		} else {
			$output .= '$min_cachePath      = \'../system/cache/min\';' . "\r\n";
		}
		$output .= '?>' . "\r\n";

		$file = fopen(DIR_SYSTEM . '../min/min.config', 'w');
		fwrite($file, $output);
		fclose($file);
	}
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->saveMinifyConfig($this->request->post);
		...
		//Tabs
		$data['tab_ips']                           = $this->language->get('tab_ips');
		
		//Heading
		$data['heading_minify_cache']              = $this->language->get('heading_minify_cache');
		$data['heading_minify_javascript']         = $this->language->get('heading_minify_javascript');
		$data['heading_minify_css']                = $this->language->get('heading_minify_css');
		$data['heading_minify_settings']           = $this->language->get('heading_minify_settings');
		$data['heading_data_caching']              = $this->language->get('heading_data_caching');
		$data['heading_html_images']               = $this->language->get('heading_html_images');
		$data['heading_content_delivery']          = $this->language->get('heading_content_delivery');
		$data['heading_speed_analyzer']            = $this->language->get('heading_speed_analyzer');
		$data['heading_block_level_caching']       = $this->language->get('heading_block_level_caching');

		//Entry
		$data['entry_google_speed_api']            = $this->language->get('entry_google_speed_api');
		$data['entry_google_server_ip']            = $this->language->get('entry_google_server_ip');
		
		$data['entry_minify_javascript']           = $this->language->get('entry_minify_javascript');
		$data['entry_javascript_jquery']           = $this->language->get('entry_javascript_jquery');
		$data['entry_javascript_jquery_version']   = $this->language->get('entry_javascript_jquery_version');
		$data['entry_javascript_jqueryui']         = $this->language->get('entry_javascript_jqueryui');
		$data['entry_javascript_jqueryui_version'] = $this->language->get('entry_javascript_jqueryui_version');
		$data['entry_javascript_defer']            = $this->language->get('entry_javascript_defer');

		$data['help_minify_javascript']            = $this->language->get('help_minify_javascript');
		$data['help_javascript_jquery']            = $this->language->get('help_javascript_jquery');
		$data['help_javascript_jquery_version']    = $this->language->get('help_javascript_jquery_version');
		$data['help_javascript_jqueryui']          = $this->language->get('help_javascript_jqueryui');
		$data['help_javascript_jqueryui_version']  = $this->language->get('help_javascript_jqueryui_version');
		$data['help_javascript_defer']             = $this->language->get('help_javascript_defer');

		
		$data['entry_css_defer']                   = $this->language->get('entry_css_defer');
		$data['entry_minify_css']                  = $this->language->get('entry_minify_css');
		$data['entry_minify_encode_images']        = $this->language->get('entry_minify_encode_images');
		$data['entry_minify_image_size']           = $this->language->get('entry_minify_image_size');
		$data['entry_minify_image_occurs']         = $this->language->get('entry_minify_image_occurs');
		$data['entry_minify_encode_url']           = $this->language->get('entry_minify_encode_url');
		$data['entry_minify_storage']              = $this->language->get('entry_minify_storage');
		$data['entry_minify_advanced']             = $this->language->get('entry_minify_advanced');
		$data['entry_minify_logging']              = $this->language->get('entry_minify_logging');
		$data['entry_minify_debug_mode']           = $this->language->get('entry_minify_debug_mode');
		$data['entry_minify_max_age']              = $this->language->get('entry_minify_max_age');
		$data['entry_minify_file_path']            = $this->language->get('entry_minify_file_path');
		$data['entry_minify_file_locking']         = $this->language->get('entry_minify_file_locking');
		$data['entry_ipscss_excludes']             = $this->language->get('entry_ipscss_excludes');
		$data['entry_ipscron_status']              = $this->language->get('entry_ipscron_status');

		$data['help_css_defer']                    = $this->language->get('help_css_defer');
		$data['help_minify_css']                   = $this->language->get('help_minify_css');
		$data['help_minify_encode_images']         = $this->language->get('help_minify_encode_images');
		$data['help_minify_image_size']            = $this->language->get('help_minify_image_size');
		$data['help_minify_image_occurs']          = $this->language->get('help_minify_image_occurs');
		$data['help_minify_encode_url']            = $this->language->get('help_minify_encode_url');
		$data['help_minify_storage']               = $this->language->get('help_minify_storage');
		$data['help_minify_advanced']              = $this->language->get('help_minify_advanced');
		$data['help_minify_logging']               = $this->language->get('help_minify_logging');
		$data['help_minify_debug_mode']            = $this->language->get('help_minify_debug_mode');
		$data['help_minify_max_age']               = $this->language->get('help_minify_max_age');
		$data['help_minify_file_path']             = $this->language->get('help_minify_file_path');
		$data['help_minify_file_locking']          = $this->language->get('help_minify_file_locking');
		$data['help_ipscss_excludes']              = $this->language->get('help_ipscss_excludes');
		$data['help_ipscron_status']               = $this->language->get('help_ipscron_status');

		
		$data['entry_category_counts']             = $this->language->get('entry_category_counts');
		$data['entry_memory_cache']                = $this->language->get('entry_memory_cache');
		$data['entry_ipsjs_excludes']              = $this->language->get('entry_ipsjs_excludes');
		$data['entry_seo_cache']                   = $this->language->get('entry_seo_cache');

		$data['help_category_counts']              = $this->language->get('help_category_counts');
		$data['help_memory_cache']                 = $this->language->get('help_memory_cache');
		$data['help_ipsjs_excludes']               = $this->language->get('help_ipsjs_excludes');
		$data['help_seo_cache']                    = $this->language->get('help_seo_cache');
		
		$data['entry_minify_html']                 = $this->language->get('entry_minify_html');
		...
		$data['entry_jpeg_compression']            = $this->language->get('entry_jpeg_compression');
		$data['entry_image_dimensions']            = $this->language->get('entry_image_dimensions');
		$data['text_image_dimensions']             = $this->language->get('text_image_dimensions');

		$data['help_minify_html']                  = $this->language->get('help_minify_html');
		$data['help_image_dimensions']             = $this->language->get('help_image_dimensions');
		$data['help_logo_dimensions']              = $this->language->get('help_logo_dimensions');
		$data['help_jpeg_compression']             = $this->language->get('help_jpeg_compression');
		$data['help_image_dimensions']             = $this->language->get('help_image_dimensions');
		
		$data['entry_cdn_status']                  = $this->language->get('entry_cdn_status');
		$data['entry_cdn_http']                    = $this->language->get('entry_cdn_http');
		$data['entry_cdn_https']                   = $this->language->get('entry_cdn_https');
		$data['entry_cdn_images']                  = $this->language->get('entry_cdn_images');
		$data['entry_cdn_js']                      = $this->language->get('entry_cdn_js');
		$data['entry_cdn_css']                     = $this->language->get('entry_cdn_css');

		$data['help_cdn_status']                   = $this->language->get('help_cdn_status');
		$data['help_cdn_http']                     = $this->language->get('help_cdn_http');
		$data['help_cdn_https']                    = $this->language->get('help_cdn_https');
		$data['help_cdn_images']                   = $this->language->get('help_cdn_images');
		$data['help_cdn_js']                       = $this->language->get('help_cdn_js');
		$data['help_cdn_css']                      = $this->language->get('help_cdn_css');

		
		$data['entry_cached_routes']               = $this->language->get('entry_cached_routes');
		$data['entry_firebug_analyzer']            = $this->language->get('entry_firebug_analyzer');
		$data['entry_firebug_queries']             = $this->language->get('entry_firebug_queries');
		
		$data['help_cached_routes']                = $this->language->get('help_cached_routes');
		$data['help_firebug_analyzer']             = $this->language->get('help_firebug_analyzer');
		$data['help_firebug_queries']              = $this->language->get('help_firebug_queries');
		...
		$data['text_google_speed_api_validate']    = $this->language->get('text_google_speed_api_validate');
		
		$data['text_memory_file']                  = $this->language->get('text_memory_file');
		$data['text_memory_apc']                   = $this->language->get('text_memory_apc');
		$data['text_memory_xcache']                = $this->language->get('text_memory_xcache');
		$data['text_memory_none']                  = $this->language->get('text_memory_none');
		$data['text_file_system']                  = $this->language->get('text_file_system');
		$data['text_in_memory_apc']                = $this->language->get('text_in_memory_apc');

		$data['text_show_advanced']                = $this->language->get('text_show_advanced');
		$data['text_hide_advanced']                = $this->language->get('text_hide_advanced');
		...
		$data['config_google_speed_api']            = $this->loadSetting('config_google_speed_api', '');
			
		$data['config_minify_javascript']           = $this->loadSetting('config_minify_javascript');
		$data['config_javascript_jquery']           = $this->loadSetting('config_javascript_jquery');
		$data['config_javascript_jquery_version']   = $this->loadSetting('config_javascript_jquery_version', '');
		$data['config_javascript_jqueryui']         = $this->loadSetting('config_javascript_jqueryui');
		$data['config_javascript_jqueryui_version'] = $this->loadSetting('config_javascript_jqueryui_version', '');
		$data['config_javascript_defer']            = $this->loadSetting('config_javascript_defer');
		$data['config_defer_javascript']            = $this->loadSetting('config_defer_javascript');
		
		$data['config_minify_css']                  = $this->loadSetting('config_minify_css');
		$data['config_minify_encode_url']           = $this->loadSetting('config_minify_encode_url');
		$data['config_minify_encode_images']        = $this->loadSetting('config_minify_encode_images');
		$data['config_minify_image_size']           = $this->loadSetting('config_minify_image_size', 3072);
		$data['config_minify_image_occurs']         = $this->loadSetting('config_minify_image_occurs', 3);
		$data['config_minify_storage']              = $this->loadSetting('config_minify_storage');
		$data['config_minify_logging']              = $this->loadSetting('config_minify_logging');
		$data['config_minify_debug_mode']           = $this->loadSetting('config_minify_debug_mode');
		$data['config_minify_max_age']              = $this->loadSetting('config_minify_max_age', 864000);
		$data['config_minify_file_path']            = $this->loadSetting('config_minify_file_path', '');
		$data['config_minify_file_locking']         = $this->loadSetting('config_minify_file_locking', true);
		$data['config_ipsjs_excludes']              = $this->loadSetting('config_ipsjs_excludes', '');
		$data['config_ipscss_excludes']             = $this->loadSetting('config_ipscss_excludes', '');
		$data['config_css_defer']                   = $this->loadSetting('config_css_defer');

		$data['config_minify_html']                 = $this->loadSetting('config_minify_html');
		$data['config_logo_width']                  = $this->loadSetting('config_logo_width');
		$data['config_logo_height']                 = $this->loadSetting('config_logo_height');
		$data['config_jpeg_compression']            = $this->loadSetting('config_jpeg_compression',90);
		
		$data['config_lazy_images']                 = $this->loadSetting('config_lazy_images');

		$data['config_category_counts']             = $this->loadSetting('config_category_counts');
		$data['config_memory_cache']                = $this->loadSetting('config_memory_cache');
		$data['config_ipscron_status']              = $this->loadSetting('config_ipscron_status', 'NOT YET RUN');	
		$data['config_seo_cache']                   = $this->loadSetting('config_seo_cache');	

		$data['config_cdn_status']                    = $this->loadSetting('config_cdn_status');	
		$data['config_cdn_http']                    = $this->loadSetting('config_cdn_http', '');	
		$data['config_cdn_https']                   = $this->loadSetting('config_cdn_https', '');
		$data['config_cdn_images']                  = $this->loadSetting('config_cdn_images');
		$data['config_cdn_js']                      = $this->loadSetting('config_cdn_js');
		$data['config_cdn_css']                     = $this->loadSetting('config_cdn_css');
		
		$data['config_cached_routes']               = $this->loadSetting('config_cached_routes', array());
		
		
		$data['config_firebug_analyzer']            = $this->loadSetting('config_firebug_analyzer');
		$data['config_firebug_queries']             = $this->loadSetting('config_firebug_queries');
