...
class ModelCatalogOption extends Model {
	...
	public function addsizeOption($data) {
		$this->event->trigger('pre.admin.option.add', $data);
		...
		foreach ($data['option_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "option_description SET option_id = '" . (int)$option_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', image = '" . $this->db->escape(html_entity_decode($data['option_image'], ENT_QUOTES, 'UTF-8')) . "', description = '" . $this->db->escape($value['description']) . "', remarks = '" . $this->db->escape($value['remarks']) . "'");
		}

		if (isset($data['option_value'])) {
			foreach ($data['option_value'] as $option_value) {
			
			    if($data['type']=='size_option') {
				        $this->db->query("INSERT INTO " . DB_PREFIX . "option_value SET option_id = '" . (int)$option_id . "', image = '', bust = '" . $this->db->escape($option_value['bust']) . "',waist = '" . $this->db->escape($option_value['waist']) . "', hip = '" . $this->db->escape($option_value['hip']) . "',hollow = '" . $this->db->escape($option_value['hollow']) . "', height = '" . $this->db->escape($option_value['height']) . "', value_type = '" . $this->db->escape($option_value['value_type']) . "', sort_order = '" . (int)$option_value['sort_order'] . "'");
				}else{
				        $this->db->query("INSERT INTO " . DB_PREFIX . "option_value SET option_id = '" . (int)$option_id . "', image = '" . $this->db->escape(html_entity_decode($option_value['image'], ENT_QUOTES, 'UTF-8')) . "', bust = '',waist = '', hip = '',hollow = '', height = '', value_type = '" . $this->db->escape($option_value['value_type']) . "', sort_order = '" . (int)$option_value['sort_order'] . "'");
				}
	...
	public function editsizeOption($option_id, $data) {
		$this->event->trigger('pre.admin.option.edit', $data);
		...
		if (isset($data['option_value'])) {
			foreach ($data['option_value'] as $option_value) {
				if ($option_value['option_value_id']) {
				    if($data['type']=='size_option') {
					$this->db->query("INSERT INTO " . DB_PREFIX . "option_value SET option_value_id = '" . (int)$option_value['option_value_id'] . "', option_id = '" . (int)$option_id . "', image = '',bust = '" . $this->db->escape($option_value['bust']) . "',waist = '" . $this->db->escape($option_value['waist']) . "', hip = '" . $this->db->escape($option_value['hip']) . "',hollow = '" . $this->db->escape($option_value['hollow']) . "', height = '" . $this->db->escape($option_value['height']) . "', value_type = '" . $this->db->escape($option_value['value_type']) . "', sort_order = '" . (int)$option_value['sort_order'] . "'");
					}else{
					$this->db->query("INSERT INTO " . DB_PREFIX . "option_value SET option_value_id = '" . (int)$option_value['option_value_id'] . "', option_id = '" . (int)$option_id . "', image = '" . $this->db->escape(html_entity_decode($option_value['image'], ENT_QUOTES, 'UTF-8')) . "',bust = '',waist = '', hip = '',hollow = '', height = '', value_type = '" . $this->db->escape($option_value['value_type']) . "', sort_order = '" . (int)$option_value['sort_order'] . "'");
					}
				} else {
				    if($data['type']=='size_option') {
					$this->db->query("INSERT INTO " . DB_PREFIX . "option_value SET option_id = '" . (int)$option_id . "', image = '',bust = '" . $this->db->escape($option_value['bust']) . "',waist = '" . $this->db->escape($option_value['waist']) . "', hip = '" . $this->db->escape($option_value['hip']) . "',hollow = '" . $this->db->escape($option_value['hollow']) . "', height = '" . $this->db->escape($option_value['height']) . "', value_type = '" . $this->db->escape($option_value['value_type']) . "', sort_order = '" . (int)$option_value['sort_order'] . "'");
	...
	public function getsizeOptionValueDescriptions($option_id) {
		$option_value_data = array();
		...
		foreach ($option_value_query->rows as $option_value) {
			...
			$option_value_data[] = array(
				...
				'option_value_description' => $option_value_description_data,
				'image'                    => $option_value['image'],
				'bust'                     => $option_value['bust'],
				'waist'                    => $option_value['waist'],
				'hip'                      => $option_value['hip'],
				'hollow'                   => $option_value['hollow'],
				'height'                   => $option_value['height'],
				'value_type'               => $option_value['value_type'],
				'sort_order'               => $option_value['sort_order']
			);
		}
	...
	public function getsizeOptionDescriptions($option_id) {
		$option_data = array();

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "option_description WHERE option_id = '" . (int)$option_id . "'");

		foreach ($query->rows as $result) {
			$option_data[$result['language_id']] = array('name' => $result['name'],'image' => $result['image'],'description' => $result['description'],'remarks' => $result['remarks']);
		}
