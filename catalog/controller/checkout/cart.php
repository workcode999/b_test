...
//echo '<pre>';print_r($product);die;
				...
				foreach ($product['option'] as $option) {
					...
					if(isset($option['custom_values'])){ $custom_values = $option['custom_values']; } else { $custom_values = false; }	
					$option_data[] = array(
						'name'  => $option['name'],
				...
				if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
					$list_price = $this->currency->format($this->tax->calculate($product['list_price'], $product['tax_class_id'], $this->config->get('config_tax')));
				} else {
					$list_price = false;
				}
		
				if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
					$save = $this->currency->format($this->tax->calculate($product['save'], $product['tax_class_id'], $this->config->get('config_tax')));
				} else {
					$save = false;
				}			
		
				if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
					$off = $this->tax->calculate($product['off'], $product['tax_class_id'], $this->config->get('config_tax'));
				} else {
					$off = false;
				}
				// Display prices
				...
				$data['products'][] = array(
					...
					'save'			=> $save,
					'off'			=> $off,
					'total_save' 	=> $this->currency->format($this->cart->getTotalSave()),
					'total'     => $total,
	...
	public function add() {
		...
		if ($product_info) {
			...
			if (isset($this->request->post['custom_option'])) {
				$custom_option = array_filter($this->request->post['custom_option']);
			} else {
				$custom_option = array();
			}
			...
			if (!$json) {//echo '<pre>';print_r($option);die;
				$this->cart->add($this->request->post['product_id'], $this->request->post['quantity'], $custom_option, $option, $recurring_id);
