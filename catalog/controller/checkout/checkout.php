...
class ControllerCheckoutCheckout extends Controller {
	public function index() {
        ...
        if($this->xtensions->isActive()){
        	$data['xheader'] = $this->load->controller('xcheckout/xheader');
			$data['xfooter'] = $this->load->controller('xcheckout/xfooter');
			$data['xcvc'] = $this->load->controller('xcheckout/xcvc');
			$modData = $this->xtensions->getModData();
        	$data['custom_js'] = html_entity_decode($modData['js'], ENT_QUOTES, 'UTF-8');
        	$this->response->setOutput($this->load->view('xtensions/template/checkout/xcheckout.tpl', $data));
         		// Xtensions Best Checkout                		
        }else if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/checkout/checkout.tpl')) {
