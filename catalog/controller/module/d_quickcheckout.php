		...
		if($this->customer->isLogged()){
			if(isset($this->session->data['payment_address']['address_id']) && $this->session->data['payment_address']['address_id'] != 0){
				...
				if(!empty($address['custom_field'])){
					foreach($address['custom_field'] as $key => $value){
						$address['custom_field_address_'.$key] = $value;
					}
				}
 ...
 */	
	private function get_login_view(){
		...
		$this->data['button_login_popup'] = $this->language->get('button_login_popup');
 ...
 */
	...
	private function shipping_same_as_payment(){
		...
		if($this->customer->isLogged() && isset($this->session->data['payment_address']) && isset($this->session->data['payment_address']['address_id']) && $this->session->data['payment_address']['address_id'] == 0){
		...
		foreach($custom_fields as $key => $custom_field){
			if($location == $custom_field['location']){
				...
				$fields['custom_field_'.$location.'_'.$custom_field['custom_field_id']] = array(
					...
					'tooltip' => '',
					'error' => array(0 => array('min_length' => 7, 
												 'max_length' => 32, 
