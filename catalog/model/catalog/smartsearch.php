...
class ModelCatalogSmartsearch extends Model {
	...
	public function smartsearch($data = array()) {
		...
		if (!empty($this->settings['replacement'])) {
			$replace = array();
			$with = array();
			...
			if (!empty($data['filter_name'])) {
				$data['filter_name'] = trim(str_replace($replace, $with, ' ' . $data['filter_name'] . ' '));
			}
			if (!empty($data['filter_tag'])) {
				$data['filter_tag'] = trim(str_replace($replace, $with, ' ' . $data['filter_tag'] . ' '));
			}
		}
		...
		if (!empty($this->settings['search_ean']) && version_compare(VERSION, '1.5.3') > 0)			$fields['p.ean'] = 'ean';
		if (!empty($this->settings['search_jan']) && version_compare(VERSION, '1.5.3') > 0)			$fields['p.jan'] = 'jan';
		if (!empty($this->settings['search_isbn']) && version_compare(VERSION, '1.5.3') > 0)		$fields['p.isbn'] = 'isbn';
		if (!empty($this->settings['search_mpn']) && version_compare(VERSION, '1.5.3') > 0)			$fields['p.mpn'] = 'mpn';
	...
	public function getProducts($smartsearch_results, $filter_data) {
		if ($filter_data['order'] == 'DESC') $smartsearch_results = array_reverse($smartsearch_results);
		$results = array_slice($smartsearch_results, $filter_data['start'], $filter_data['limit']);
