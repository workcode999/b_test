		...
		if (isset($data['products'])) {
			foreach ($data['products'] as $product) {
				...
				foreach ($product['option'] as $option) {
					if(isset($option['custom_values']['remarks'])) { $remarks = $option['custom_values']['remarks']; }else{ $remarks ='';}
					$this->db->query('INSERT INTO ' . DB_PREFIX . 'order_option SET order_id = "' . (int)$order_id . '", order_product_id = "' . (int)$order_product_id . '", product_option_id = "' . (int)$option['product_option_id'] . '", product_option_value_id = "' . (int)$option['product_option_value_id'] . '", name = "' . $this->db->escape($option['name']) . '", `value` = "' . $this->db->escape($option['value']) . '", `type` = "' . $this->db->escape($option['type']) . '",`bust` = "' . $option['custom_values']['Bust'] . '", `waist` = "' . $option['custom_values']['Waist'] . '", `hip` = "' . $option['custom_values']['Hip'] . '", hollow = "' . $option['custom_values']['Hollow'] . '", `height` = "' . $option['custom_values']['Height'] . '", `remarks` = "' . $remarks .'"');
				}
			}
		}
		...
		if ($order_info) {
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
				...
				if ($order_info['customer_id']) {
					...
					$data['text_link'] = $language->get('text_guest_link');
				}
				...
				$data['text_bust'] = $language->get('text_bust');
				$data['text_waist'] = $language->get('text_waist');
				$data['text_hip'] = $language->get('text_hip');
				$data['text_hollow'] = $language->get('text_hollow');
				$data['text_height'] = $language->get('text_height');
			    $data['text_size_selected'] = $language->get('text_size_selected');
				$data['text_size_details'] = $language->get('text_size_details');
				$data['text_remarks'] = $language->get('text_remarks');
				$data['text_submit'] = $language->get('text_submit');
				$data['text_free'] = $language->get('text_free');			
				$data['text_cancel'] = $language->get('text_cancel');
				...
				if ($order_info['customer_id']) {
				  ...
				  if (isset($this->request->server['HTTPS']) && (($this->request->server['HTTPS'] == 'on') || ($this->request->server['HTTPS'] == '1'))) {			
						...
						$data['link'] = $storelink . 'index.php?route=account/guest';
				   } else {
						$data['link'] = $order_info['store_url'] . 'index.php?route=account/guest';
				   }
				}
				...
				if($this->xtensions->isActive()){
					...
					$fieldData = $this->model_account_xcustom_field->getCustomFieldByIdentifier($order_info['custom_field'],'account');
					foreach($this->model_account_xcustom_field->getCustomFields($order_info['customer_group_id']) as $field){
			...
			if($notify == 1){
				if ($order_info['order_status_id'] && $order_status_id) {
					...
					if ($order_info['customer_id']) {
						$message .= $language->get('text_new_link') . "\n";
						$message .= $order_info['store_url'] . 'index.php?route=account/order/info&order_id=' . $order_id . "\n\n";
					} else {
						$message .= $language->get('text_guest_link') . "\n";
						$message .= $order_info['store_url'] . 'index.php?route=account/guest' . "\n\n";
					}
