...
<!--			<?php if(isset($extra_field) && $extra_field) { ?>
          ...
          <ul class="list-unstyled">
            ...
            <?php if ($points) { ?>
            <?php } ?>
		<li><script type="text/javascript" src="http://www.bjsbridal.com/catalog/view/javascript/jquery/kkcountdown.js"></script><div class="coupon">Enjoy Extra <b>$5</b> Off with Coupon Code: <b>BJSOFF5</b> and Free Shipping on Order Over <b>$79</b>.</div><div class="count_down"><span time="<?php echo strtotime("2016-12-7"); ?>" class="kkcount-down-2"></span></div></li>
            <?php if ($discounts) { ?>
            <li>
              <hr>
            </li>
...
<div class="size-content tabTwo">
			...
			<div class="addQuantity">
				<label class="control-label label-change" for="input-quantity"><?php echo $entry_qty; ?></label>
				<button class="myMinus" disabled="disabled"><i class="fa">-</i></button>
			    <input type="text" name="quantity" value="<?php echo $minimum; ?>" size="2" id="input-quantity" />
				<button class="myPlus"><i class="fa">+</i></button>
			</div>
                <input type="hidden" name="product_id" value="<?php echo $product_id; ?>" />
				<div>
					<button type="button" id="quick-buy" data-loading-text="<?php echo $text_loading; ?>" class="btn btn-primary btn-lg" style="width:49.4%;">QUICK BUY</button>
...
<div class="tab-pane" id="tab-askquestion">
  ...
  <?php } ?>
      ...
      <?php if ($products) { ?>
      <div class="col-sm-2 related-row">
	  <div class="row">
	...
	<div class="col-md-6 col-xs-6">
	  <div>
		...
		<?php } else { ?>
		<span class="related-new"><?php echo $product['special']; ?></span>
		<?php } ?>
		<?php if ($product['tax']) { ?>
...
$('#button-review').on('click', function() {
	...
	$.ajax({
		...
		type: 'post',
		dataType: 'json',
		data: 'name=' + encodeURIComponent($('input[name=\'name\']').val())+'&email=' + '&email=' +encodeURIComponent($('input[name=\'email\']').val()) + '&text=' + encodeURIComponent($('textarea[name=\'text\']').val()) + '&rating=' + encodeURIComponent(num),//encodeURIComponent($('input[name=\'rating\']:checked').val() ? $('input[name=\'rating\']:checked').val() : '') '&captcha=' + encodeURIComponent($('input[name=\'captcha\']').val()
		beforeSend: function() {
