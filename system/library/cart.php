...
class Cart {
	...
	public function getProducts() {
		if (!$this->data) {
			foreach ($this->session->data['cart'] as $key => $quantity) {
				...
				if (!empty($product['custom_option'])) {
				...
				if ($product_query->num_rows) {
					...
					foreach ($options as $product_option_id => $value) {
						...
						if ($option_query->num_rows) {
							if(isset($custom_option[$product_option_id])) { $custom_values = $custom_option[$product_option_id]; } else { $custom_values = false; }
							if ($option_query->row['type'] == 'select' || $option_query->row['type'] == 'radio' || $option_query->row['type'] == 'size_option' || $option_query->row['type'] == 'image') {
					...
					$list_price = $product_query->row['price'];
					...
					$this->data[$key] = array(
						...
						'save'         => ($list_price - $price),
						'off'          => 100 * round(1 - ($price / $list_price),2),
						'subtotal_save'   => ($list_price - $price) * $quantity,
	...
	public function add($product_id, $qty = 1, $custom_option, $option = array(), $recurring_id = 0) {
		$this->data = array();
	...
	public function getTotalSave() {
			$total_save = 0;
			
			foreach ($this->getProducts() as $product) {
				$total_save += $this->tax->calculate($product['subtotal_save'], $product['tax_class_id'], $this->config->get('config_tax'));
			}
