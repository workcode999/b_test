...
class ControllerProductCategory extends Controller {
	public function index() {
		...
		if ($category_info) {
			...
			$results = array();
			if ($this->config->get('sphinx_search_module') && $this->config->get('sphinx_category_status') && empty($filter) && empty($area)) { //XSP: modify
	                $this->load->model('catalog/sphinx');
	                // Search and import result keys to variable space ($product_total and $results);
	                $results = $this->model_catalog_sphinx->search($filter_data);
	                extract($results);
	        } else {
			...
			$data['popular'] = $this->url->link('product/product', 'path=' . $this->request->get['path']);
			...
			$data['results'] = ceil($product_total / $limit);
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/product/category.tpl')) {
				$this->response->setOutput(magicRender($this->load->view($this->config->get('config_template') . '/template/product/category.tpl', $data),$this,'category',(isset($results) ? $results : array())));
			} else {
				$this->response->setOutput(magicRender($this->load->view('default/template/product/category.tpl', $data),$this,'category',(isset($results) ? $results : array())));
			}
		} else {
			$url = '';
