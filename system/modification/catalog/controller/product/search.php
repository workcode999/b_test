...
class ControllerProductSearch extends Controller {
	public function index() {
		...
		if (isset($this->request->get['search'])) {
			...
			$this->document->setTitle($this->language->get('heading_tag') . $this->request->get['tag']) .  ' - ' . $this->language->get('heading_title');
		} else {
		...
		if (isset($this->request->get['search']) || isset($this->request->get['tag'])) {
		  ...
		  if ($this->config->get('sphinx_search_module')) {
                ...
                if ($product_total == 0) {
                    $data['search_suggestion'] = $this->model_catalog_sphinx->buildSuggestion($filter_data['filter_name']);
			...
			/*XSP: add*/
			if(isset($this->request->get['sort']) == 'p.price' && $this->request->get['order'] == 'DESC'){
				$data['sorts'] = array(
					'text'  => 'defalut',
					'value' => 'defalut',
					'title'  => 'Click restore default again',
					'href'  => $this->url->link('product/search',$url)
			
				);
			}else if(isset($this->request->get['sort']) == 'p.price' && $this->request->get['order'] == 'ASC'){
				$data['sorts'] = array(
					'text'  => $this->language->get('text_price_desc'),
					'value' => 'p.price-desc',
					'title'  => 'Click again from high to low',
					'href'  => $this->url->link('product/search', 'sort=p.price&order=DESC' . $url)
				...
				$data['sorts'] = array(
					'text'  => $this->language->get('text_price_asc'),
					'value' => 'p.price-asc',
					'title'  => 'Click again from low to high',
					'href'  => $this->url->link('product/search','sort=p.price&order=ASC' . $url)
			
				);
			}
			$data['viewed'] = array();
			if(isset($this->request->get['sort']) == 'p.viewed' && $this->request->get['order'] == 'DESC'){
				$data['viewed'] = array(
					'title'  => 'Click again from low to high',
					'href'  => $this->url->link('product/search',$url)
			
				);
			}elseif (isset($this->request->get['sort']) == 'p.viewed' && $this->request->get['order'] == 'ASC'){
				$data['viewed'] = array(
					'title'  => 'Click restore default again',
				...
				$data['viewed'] = array(
					'title'  => 'Click restore default again',
					'href'  => $this->url->link('product/search','&sort=p.viewed&order=ASC' . $url)
			...
			$pagination1->url = $this->url->link('product/search', $url . '&page={page}');

			$data['pagination1'] = $pagination1->render();			

			$data['results'] = ceil($product_total / $limit);$data['resultsImg'] = $product_total;
		}
		$data['breadcrumbs'][] = array(
			'text' => $search,
			'href' => $this->url->link('product/search', $url)
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/product/search.tpl')) {
			$this->response->setOutput(magicRender($this->load->view($this->config->get('config_template') . '/template/product/search.tpl', $data),$this,'search',(isset($results) ? $results : array())));
		} else {
